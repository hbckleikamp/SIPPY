...
def EleCounter(mdf, # dataframe, requires a column titled "peptide"
...
#%%
    ...
    #add water loss
    pep_comp["H"]=pep_comp["H"]+2
    pep_comp["O"]=pep_comp["O"]+1
    ...
    comp=comp[comp.sum()[comp.sum()>0].index] #remove elements with zero counts
    elements=comp.columns.tolist()
