...
def IDFileParser(file,                     # should be single file (pepXML, idXML or mzID)
    ...
    #specdf=specdf.explode("protein")
    
    if write_file: 
        if output_folder==None: #write in same folder as input file
            output_file=str(Path(Path(IDFile).parents[0],Path(IDFile).stem+".tsv"))
        else:
            ...
            output_file=str(Path(output_folder,Path(IDFile).stem+".tsv"))
        specdf.to_csv(output_file,sep="\t")
        return output_file
